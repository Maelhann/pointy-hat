...
const TEMPLATES: Record<string, object> = {
  blank: {
    spell: {
      ...
      steps: [],
      outputs: [
        { id: "result", type: "document", format: ["md"], description: "Describe what this spell produces" },
      ],
      effects: [],
      metadata: {},
    },
      ...
      outputs: [
        {
          ...
          type: "document",
          format: ["md"],
          description: "Detailed code review with specific findings and suggestions",
          acceptance_criteria: "Must include specific code references, severity ratings, and actionable suggestions",
          quality_check: {
      ...
      outputs: [
        {
          ...
          type: "document",
          format: ["md"],
          description: "Comprehensive analysis with key findings and recommendations",
          acceptance_criteria: "Must include specific numbers, at least 3 key findings, and actionable recommendations",
          quality_check: {
      ...
      outputs: [
        {
          ...
          type: "document",
          format: ["md", "pdf"],
          description: "Professional report with executive summary and recommendations",
          acceptance_criteria: "Professionally formatted, uses specific data points, includes actionable recommendations",
          quality_check: {
  ...
  program
    ...
    .action(async (name: string | undefined, opts: { template: string; interactive?: boolean }) => {
      try {
        ...
        console.log(chalk.dim(`  pointyhat validate ${fileName}`));
        console.log(chalk.dim(`  pointyhat cast ${fileName} --dry-run`));
        console.log();
      } catch (err) {
        handleError(err);
      }
